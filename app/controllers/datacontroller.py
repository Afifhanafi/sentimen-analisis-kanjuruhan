...
from app.models import DatasetBefore
from app.models import DatasetAfter
import pandas as pd
...
#Dataset Setelah Tragedi Kanjuruhan
class DataController:
    def importDatasetAfter(self, file):
        try:
            readCSV = pd.read_csv(file, sep=';', encoding='unicode_escape')
            for row in range(0, len(readCSV)):
                dataset = app.models.DatasetAfter(
                    created_at = readCSV["created_at"][row],
                    username = readCSV["username"][row],
                    label = readCSV["label"][row],
                    raw_tweets = readCSV["tweet"][row]
        ...
        def retrieveDataAfter(self):
            return DatasetAfter.query.all()
    
        def retrieveCountDataAfter(self):
            return DatasetAfter.query.count()
